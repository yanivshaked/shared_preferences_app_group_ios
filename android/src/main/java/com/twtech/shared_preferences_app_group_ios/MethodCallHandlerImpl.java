...
class MethodCallHandlerImpl implements MethodChannel.MethodCallHandler {
  ...
  private static final String MAP_IDENTIFIER =     "VGhpcyBpcyB0aGUgcHJlZml4IGZvciBhIG1hcC4=";
    ...
    try {
      switch (call.method) {
        ...
        case "setString":
          ...
          if (value.startsWith(LIST_IDENTIFIER) || value.startsWith(BIG_INTEGER_PREFIX) || value.startsWith(DOUBLE_PREFIX) || value.startsWith(MAP_IDENTIFIER)) {
            result.error(
                "StorageError",
        ...
        case "setStringList":
          ...
          break;
        case "setMap":
          HashMap<String, String> hashMap = call.argument("value");
          commitAsync(
              preferences.edit().putString(key, MAP_IDENTIFIER + encodeHashMap(hashMap)), result);
          break;
        case "commit":
  ...
  private HashMap<String, String> decodeHashMap(String encodedHashMap) throws IOException {
    ObjectInputStream stream = null;
    try {
      stream = new ObjectInputStream(new ByteArrayInputStream(Base64.decode(encodedHashMap, 0)));
      return (HashMap<String, String>) stream.readObject();
    } catch (ClassNotFoundException e) {
      throw new IOException(e);
    } finally {
      if (stream != null) {
  ...
  private String encodeHashMap(HashMap<String, String> hashMap) throws IOException {
    ObjectOutputStream stream = null;
    try {
      ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
      stream = new ObjectOutputStream(byteStream);
    ...
    for (String key : allPrefs.keySet()) {
      if (key.startsWith("flutter.")) {
        ...
        if (value instanceof String) {
          ...
          } else if (stringValue.startsWith(MAP_IDENTIFIER)) {
            value = decodeHashMap(stringValue.substring(MAP_IDENTIFIER.length()));
